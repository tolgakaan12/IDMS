...
def main():
    ...
    sns.set()
    data = slu.load_emg(
        "/Users/tolgakaancelebi/VS Code/IDMS/Processed/AngularStops01.json")
    ...
    print("Shape:", emg_data.shape)
        ...
        # r'C:\Users\hbkm9\Documents\Projects\CYB\Experiment3\Processed\NDMS01_Processed\Data\AngularWalk01.json')
    # average = iterative_demean(data[0, :], mu=0.0008)
    # plt.plot(data[0, :] - np.mean(data[0, :]))
    # plt.plot(average)
    # plt.hlines([np.mean(data[0, :]), np.mean(data[0, :]) + 0.0005], 0, data.shape[1], colors='r', ls='--')
    # plt.show()
    # mav_emg = mav(data, 201)

    # plt.plot(np.abs(norm_emg(data))[0, :])
    # plt.plot(mav_emg[0, :])
    # plt.show()
    pass
